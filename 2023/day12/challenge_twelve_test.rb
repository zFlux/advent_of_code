...
class ChallengeTwelve_Objects < Minitest::Test
    ...
    def test_find_counts_1
        input = "???.### 1,1,3"
        spring_group = SpringGroup.new(input)
        assert_equal 1, spring_group.count_of_possibilities
    end

    def test_find_counts_2
        input = ".??..??...?##. 1,1,3"
        spring_group = SpringGroup.new(input)
        assert_equal 4, spring_group.count_of_possibilities
    end
    ...
    def test_find_counts_6
        input = "?###???????? 3,2,1"
        spring_group = SpringGroup.new(input)
        assert_equal 10, spring_group.count_of_possibilities
    end

    def test_find_counts_7
        input = "???????????? 3,2,1"
        spring_group = SpringGroup.new(input)
        assert_equal 35, spring_group.count_of_possibilities
    end
    ...
    def test_find_counts_8
        ...
        spring_group = SpringGroup.new(input)
        assert_equal 3, spring_group.count_of_possibilities
    end
    ...
    def test_find_counts_unfolded_2
        input = ".??..??...?##. 1,1,3"
        spring_group = SpringGroup.new(input, true)
        assert_equal 16384, spring_group.count_of_possibilities
    end
    ...
    def test_find_counts_unfolded_4
        input = "????.#...#... 4,1,1"
        spring_group = SpringGroup.new(input, true)
        assert_equal 16, spring_group.count_of_possibilities
    end

    def test_find_counts__unfolded_5
        input = "????.######..#####. 1,6,5"
        spring_group = SpringGroup.new(input, true)
        assert_equal 2500, spring_group.count_of_possibilities
    end

    def test_find_counts_unfolded_6
        input = "?###???????? 3,2,1"
        spring_group = SpringGroup.new(input, true)
        assert_equal 506250, spring_group.count_of_possibilities
    end
end
