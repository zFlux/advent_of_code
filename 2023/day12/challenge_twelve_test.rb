...
class ChallengeTwelve_Objects < Minitest::Test
    ...
    def test_find_first_group_positions
        input = "?###??????????###??????????###??????????###??????????###???????? 3,2,1,3,2,1,3,2,1,3,2,1,3,2,1"
        spring_group = SpringGroup.new(input)
        assert_equal [1, 5, 8, 14, 18, 21, 27, 31, 34, 40, 44, 47, 53, 57, 60], spring_group.start_points
    end
    ...
    def test_find_first_group_positions_3
        input = "???.### 1,1,3"
        spring_group = SpringGroup.new(input)
        assert_equal [0, 2, 4], spring_group.start_points
    end

    def test_find_first_group_positions_4
        input = ".??..??...?##. 1,1,3"
        spring_group = SpringGroup.new(input)
        assert_equal [1, 5, 10], spring_group.start_points
    end
    ...
    def test_find_first_group_positions_5
        ...
        #        .#.###.#.######
        spring_group = SpringGroup.new(input)
        assert_equal [1, 3, 7, 9], spring_group.start_points
    end

    def test_find_first_group_positions_6
        input = "????.#...#... 4,1,1"
        spring_group = SpringGroup.new(input)
        assert_equal [0, 5, 9], spring_group.start_points
    end

    def test_find_first_group_positions_7
        input = "????.######..#####. 1,6,5"
        spring_group = SpringGroup.new(input)
        assert_equal [0, 5, 13], spring_group.start_points
    end
    
    def test_find_possibilities
        input = "?###??????????###??????????###??????????###??????????###???????? 3,2,1,3,2,1,3,2,1,3,2,1,3,2,1"
        spring_group = SpringGroup.new(input)
        assert_equal 1, spring_group.find_possibilities
    end
